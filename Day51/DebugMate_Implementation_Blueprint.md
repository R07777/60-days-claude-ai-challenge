...
**Status:** Single source of truth for Days 2–10. Generated on Day 1 after requirements were finalized (see companion PRD).
...
1. **Tech stack decision.** Recommended default (confirm or adjust based on your comfort and constraints): **React (frontend) + Node.js/Express (backend) + MongoDB (database) + Claude API (diagnosis engine) + JWT for auth sessions.** This matches your existing MERN experience and keeps the whole stack in JavaScript, which minimizes context-switching in a tight timeline. Alternatives to consider and reject/accept explicitly: Django+Postgres (more setup overhead for this timeline), Next.js full-stack (viable, slightly different mental model, ok if you prefer one deployable unit). **Decision must be written down explicitly** at the top of the Day 2 progress log so every later day can assume it.
...
- A free-tier hosting service for the backend and one for the frontend (chosen together on Day 3 based on what's genuinely free and simple to connect to GitHub for auto-deploy)
...
### 🔗 APIs, libraries, services, or tools to integrate
- Your chosen free-tier hosting providers (from Day 3) for frontend and backend
